...
function drop (e) {
    ...
    $.ajax({
        type: 'POST',
        url: `http://127.0.0.1:5000/play-human/${bodyEl.id}/${move_list.length}`,
        ...
        success: function (data) {
            ...
            if (data['answer'] === 'true') {
                ...
                // }
                castling(parent.id+new_id, data);
...
$('document').ready(() => {
    ...
    setInterval(computerPlayer(), 1000);
    ...
    if (bodyEl.classList.contains('classical')) {
        ...
        if (bodyEl.classList.contains('False')) {
            oppTimer();
        } else {
            playerTimer();
        }
